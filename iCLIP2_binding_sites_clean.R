...
myfiles = list.files(mypath, pattern = paste0(sampleX, ".*.bw$"), full.names = T)
...
### Load binding sites
bindingsites = paste0('results/', sampleX, "_peaks_curated.bed")
...
aa = data.frame(cutoff = c(quantile(bs$clp_rep1, probs = seq(0,1, by = 0.1))[2],
                ...
                variable = c("clp_rep1", "clp_rep2", "clp_rep3"))
### Add lower boundary 
aa$cutoff[aa$cutoff < 2] = 2

### Make per replicate distribution plot
pdf(paste0('figure/', sampleX, '_per_replicate_distribution_plot.pdf'))
...
### Make summary plot
pdf(paste0('figure/', sampleX, '_summary_plot.pdf'))
...
pdf(paste0('figure/', sampleX, '_gene_perspective.pdf'))
...
pdf(paste0('figure/', sampleX, '_binding_site_perspective.pdf'))
ggplot(df, aes(x = reorder(df[,1], -df[,2]), y = df[,2])) + 
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
pdf(paste0('figure/', sampleX, '_binding_site_annotation_overlaps.pdf'))
...
pdf(paste0('figure/', sampleX, '_number_of_different_transcript_annotaitons_at_the_same_position.pdf'))
...
pdf(paste0('figure/', sampleX, '_RBP_location.pdf'))
...
### Write bed file
export(bs.clean, paste0('results/', sampleX, '_binding_site_clean.bed'), format = "BED")
